...
import { NavigationExpression } from '../../src/ast/expressions';
...
describe('Feittype', () => {
  ...
  test('should navigate through feittype relationships', () => {
    ...
    const navExpr: NavigationExpression = {
      type: 'NavigationExpression',
      object: { type: 'VariableReference', variableName: 'vlucht' },
      attribute: 'passagiers'
    };
    
    // Set the current vlucht as context
    context.setVariable('vlucht', vluchtObj);
    
    const passagiers = evaluator.evaluate(navExpr, context);
