...
describe('Engine - Binary Expressions', () => {
  ...
  describe('Subtraction', () => {
    ...
    test('should evaluate chain subtraction', () => {
      const result = engine.run('10 - 3 - 2');
      expect(result.success).toBe(true);
      expect(result.value).toEqual({
        type: 'number',
        value: 5
  ...
  describe('Operator Precedence', () => {
    ...
    test('should handle division before subtraction', () => {
      const result = engine.run('20 - 12 / 3');
      expect(result.success).toBe(true);
      expect(result.value).toEqual({
        type: 'number',
    ...
    test('should handle complex expression', () => {
      const result = engine.run('2 * 3 + 4 * 5');
      expect(result.success).toBe(true);
      expect(result.value).toEqual({
        type: 'number',
        value: 26  // 6 + 20
  ...
  describe('Parentheses', () => {
    ...
    test('should fail on missing closing parenthesis', () => {
      const result = engine.run('(2 + 3');
      expect(result.success).toBe(false);
      expect(result.error?.message).toBe('Expected closing parenthesis');
    });
  });
